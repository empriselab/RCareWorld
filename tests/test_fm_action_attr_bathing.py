...
def main():
    parser = argparse.ArgumentParser(description="Test FMActionAttr bathing actions.")
    parser.add_argument("--port", type=int, default=5004)
    parser.add_argument("--simulate", action="store_true")
    parser.add_argument("--id", type=int, default=None)
    parser.add_argument("--name", type=str, default="BathingRemote")
    args = parser.parse_args()

    # Ensure Unity class name "BathingRemote" is recognized as FMActionAttr.
    attr.attrs["BathingRemote"] = FMActionAttr
    ...
    if target is None:
        print("No BathingRemote attr found. Available attrs:")
        for attr_id, inst in env.attrs.items():
            name = inst.data.get("name", "")
    ...
    actions = [
        ("bath_right_arm", lambda: target.wipe("right_arm", 0.1)),
        ("bath_right_leg", lambda: target.wipe("right leg", 0.1)),
        ("ot_left", lambda: target.ot_left()),
        ("ot_right", lambda: target.ot_right()),
        ("bath_left_leg", lambda: target.wipe("left leg", 0.1)),
        ("bath_left_arm", lambda: target.wipe("left_arm", 0.1)),
    ]
