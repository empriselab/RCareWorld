...
def main():
    ap = argparse.ArgumentParser(description="Test PersonRandomizerAttr methods")
    default_player = (
    ...
    ap.add_argument("--name", default="PersonRandomizerTestObject")
    ...
    try:
        ...
        pr = env.InstanceObject(name=args.name, attr_type=attr.PersonRandomizerAttr)
        print(f"Bound PersonRandomizerAttr to '{args.name}'")

        # SetSeed
        pr.SetSeed(args.seed); step(env, 3); print("SetSeed executed")

        print("\nAll PersonRandomizerAttr methods invoked successfully")
    finally:
        env.Pend(); env.close()
