...
for i in range(num_frames):
    ...
    depth_exr = cv2.imdecode(np.frombuffer(depth_bytes, dtype=np.uint8), cv2.IMREAD_UNCHANGED)
...
for i in range(num_frames):
    ...
    with open(f"data/pose_{i}.txt", "w") as f:
        f.write(f"{position[0]} {position[1]} {position[2]} {quaternion[0]} {quaternion[1]} {quaternion[2]} {quaternion[3]}")
